const swiper = new Swiper('.swiper', {
    ...
    const getSliders = async () => {
      const { data } = await axios.get(`${url}slider/active`);
      return data.sliders;
    }
    const displaySliders = async () => {
      ...
      `);
      document.querySelector(".swiper-wrapper").innerHTML = result.join('');
      swiper.update();
    }
    displaySliders();


    const displayCategoriesHomePage = async () => {
        const categories = await getHeaderCategories();
        const result = categories.map((category)=>
        `<div class="col-lg-6">
            <a href="category.html?id=${category._id}"">
    ...
    displayCategoriesHomePage();
    const getMostBorrowed = async()=>{
        const {data} = await axios.get(`${url}book/all?limit=8&sort=-numberBorrow`);
        return data.books;
     }

     const displayMostBorrowed = async()=>{
        const books = await getMostBorrowed();
        const result = books.map ((book)=>`
            ...
            <div class="book card gap-3">
                <div class="card-header d-flex flex-column gap-2">
                    ...
                    <h4 class="card-subtitle"> مرات الإستعارة : ${book.numberBorrow} </h4>
                </div>
                <div class="card-body">
        ...
        document.querySelector(".popularBorrowed").innerHTML=result;
     }

     const getHomePageBooks = async()=>{
        document.querySelector(".overlay").classList.remove('d-none');
        const {data} =await axios.get(`${url}book/all?limit=8`);
        return data.books;
     }
     const displayHomePageBooks = async()=>{
        ...
        document.querySelector("#chose-books").innerHTML=result;
